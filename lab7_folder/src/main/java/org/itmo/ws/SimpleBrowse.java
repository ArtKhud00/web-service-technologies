...
public class SimpleBrowse {
    ...
    public SimpleBrowse() {
        try {
        	...
        	UDDIClient client = new UDDIClient("META-INF/simple-uddi.xml");
    ...
    public void Browse(String username, String password) {
        try {
            String token = GetAuthKey(username, password, AuthStyle.UDDI_AUTH);
    ...
    public String searchService(String serviceName, String username, String password) throws Exception {
        String token = GetAuthKey(username, password, AuthStyle.UDDI_AUTH);
        BusinessList findBusiness = GetBusinessList(token);
        BusinessInfos findBusinessInfos = findBusiness.getBusinessInfos();
        int size = findBusinessInfos.getBusinessInfo().size();
        for(int i = 0; i < size; ++i){
            GetServiceDetail getServiceDetail = new GetServiceDetail();
            try{
                for(int j = 0;
                    j < findBusinessInfos.getBusinessInfo().get(i).getServiceInfos().getServiceInfo().size();
                    ++j){
                    getServiceDetail.getServiceKey().add(findBusinessInfos.getBusinessInfo()
                                                        .get(i)
                                                        .getServiceInfos()
                                                        .getServiceInfo()
                                                        .get(j)
                ...
                for(int k = 0; k < serviceDetail.getBusinessService().size(); ++k){
                    BusinessService businessService = serviceDetail.getBusinessService().get(k);
                    String tempServiceName = ListToString(businessService.getName());
                    if(tempServiceName.equals(serviceName)){
                        System.out.println("Access point to service is: " + findBusinessInfos
                                                                            .getBusinessInfo()
                                                                            .get(i)
                                                                            .getBusinessKey());
                        return getServiceAccessPoint(businessService.getBindingTemplates());
                    }
                }
            }
            catch (NullPointerException ex){
                System.out.println("Nothing was found, " + ex);
                return null;
            }
        }
        return null;
